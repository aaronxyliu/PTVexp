    ...
    for entry in res:
        ...
        if libs:
            for lib in libs:
                ...
                if 'date' in lib and lib['date'] != '':
                    avg_release_time_dist.add(table_name, lib['date'])

            conn.update(table_name, ['result'], (json.dumps(libs),), f"`id`='{id}'")

        # logger.info(f'{id} finished.')
    logger.info(f'=== CATEGORY: {table_name} ===')
    logger.info(f'# Libs: {lib_cnt}')
    logger.info(f'# Versions: {version_cnt}')
    logger.info(f'# Websites: {web_cnt}')
    logger.info(f'# Libs per website: {round(lib_cnt / web_cnt, 1)}')
...
if __name__ == '__main__':
    # Usage: python3 analyze/lib_release_time_induce2.py
    for table_name in conn.show_tables():
        analyze(table_name)
    # logger.info(avg_release_time_dist.dict)
    logger.info(avg_release_time_dist.avgDateDict('Average Release Time of Each Category'))
    conn.close()
    conn2.close()